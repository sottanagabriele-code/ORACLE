...
REFRESH_SECS = int(os.environ.get("REFRESH_SECS", "120"))
...
# Cache globale
_cache: Dict[str, Any] = {"markets": [], "last_update": None, "session": None}
...
async def fetch_resolved_markets(limit: int = 300) -> List[Dict[str, Any]]:
...
async def analyze_market_sentiment(question: str):
    ...
    for raw in (gdelt if isinstance(gdelt, list) else []):
        ...
        try:
            ...
            pass
        recency = math.exp(-0.693 * age_h / 12)  # half-life 12h
        articles.append(
            {
                "title": title,
                "sentiment": s,
...
def oracle_predict(poly_prob: float, sentiment_lr: float, ai_prob: Optional[float] = None, velocity: float = 0.0, vol_spike: float = 1.0):
...
def get_recommendation(edge_pct: float, confidence: float):
    if edge_pct > 5 and confidence > 0.55:
        ...
        return {"action": "BUY_YES", "label": "▲ COMPRA YES", "strength": strength}
    if edge_pct < -5 and confidence > 0.55:
        strength = "FORTE" if edge_pct < -10 else "MODERATO"
        return {"action": "BUY_NO", "label": "▼ COMPRA NO", "strength": strength}
...
async def process_market(raw: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    m = parse_market(raw)
...
async def run_backtest(days_back: int = 90):
    print("[Backtest] Fetching resolved markets…")
